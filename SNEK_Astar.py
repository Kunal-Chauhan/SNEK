        ...
        for neighbor in neighboring:
            if neighbor not in closed_list:
                if neighbor not in open_list:
                    ...
                    layer[str(neighbor)] = layer[str(
                        parent[str(neighbor)])] + 1
...
def neighbors(node, walls):
    ...
    if above not in wall_positions and above[1] < 290:
        neighboring.append(above)
    if below not in wall_positions and below[1] > -290:
        neighboring.append(below)
    if onright not in wall_positions and onright[0] < 290:
        neighboring.append(onright)
    if onleft not in wall_positions and onleft[0] > -290:
        neighboring.append(onleft)
    return neighboring
...
while True:
    ...
    if head.pos() == food.pos():
        ...
        if score > high_score:
            high_score = score
        pen.clear()
        pen.write('Score: '+str(score)+'  High Score: '+str(high_score),
